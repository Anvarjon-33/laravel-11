...
addEventListener('alpine:init', () => {
    ...
    window.addEventListener('DOMContentLoaded', () => {
        ...
        window.Echo.private(`update.logo.${window.laravel.user}`).listen('.logo-updated', (val) => {
            console.log('PRIVATE CHANNEL IS WORK !!!')
            console.log(val)
        })

        window.Echo.private(`testing.channel.${window.laravel.user}`).listen('.testing.event', (val) => {
